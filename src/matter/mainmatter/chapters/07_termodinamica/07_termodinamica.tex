...
\begin{tcolorbox}[colback = yellow!30, colframe = yellow!30!black, title = {Trasformazione quasistatica}]
    ...
    attraversa equilibri successivi. Data una trasformazione $\gamma$ quasistatica
    sul piano $pV$, tutti i punti di $\gamma$ sono stati di equilibrio.
\end{tcolorbox}
...
mediante una certa costante dalla relazione $\Delta T = \gamma Q$.
...
comprare e bruciare, a meno della costante $\gamma$ espressa precedentemente.
...
\[ Q = \frac{1}{\gamma}\Delta T = \frac{1}{\gamma}(T_f - T_i) = \frac{1}{\gamma}(T(U_f) - T(U_i)) \]
...
$\gamma$, il lavoro totale di un gas può essere espresso nella
seguente forma:

\begin{align}
    W_\gamma = \int_\gamma pdV
\end{align}

\noindent Si noti che la pressione $p$ dovrà essere
una qualche funzione esprimibile secondo altre variabili termodinamiche,
...
Osserviamo infine che $c_p > c_V$. È facile spiegare intuitivamente
...
\[ \Delta U_\gamma = 0 \quad \forall \gamma \text{ ciclica} \]
...
\begin{enumerate}
    ...
    sia quasistatica. Questo tratto è quello che immette calore
...
\noindent Sembrerebbe che la teoria sia fondata: lavoro e calore sono
convertibili, cioè spendendo il calore otteniamo lavoro e possiamo tra l'altro
...
seguente evidenza: \textit{il calore non fluisce mai spontaneamente
da un corpo ad uno più caldo}.
