...
Notare che è sempre necessario specificare il ``percorso'' $\gamma$
...
Il percorso prende il nome \textit{trasformazione termodinamica}
e studieremo ora i più semplici. Ricordiamo inoltre che le
...
Sappiamo dagli studi di calorimetria che possiamo associare al
...
In generale, avendo $l$ gradi di libertà vale $c_V = \frac{l}{2}R$.
...
Dalle trasformazioni isocore abbiamo concluso che $dU = nc_VdT$.
...
Riproponiamo l'equazione di stato dei gas ideali $pV = nRT$ e
...
Dunque $dU = nc_pdT - nRdT = nc_vdT$, da cui la seguente relazione.

\begin{align}
    c_p - c_v = R
\end{align}

Quella che abbiamo appena ottenuto è la cosiddetta \textit{relazione
...
Incontreremo di nuovo il rapporto $\gamma$. Da questo risultato
risulta dunque che

\[ c_p > c_V \]

sempre (entro il nostro modello). Ma come mai è importante precisare
...
costante e ciò significa che il gas deve esercitare una pressione
costante durante la trasformazione. Ciò comporta la necessità di un
...
Nelle trasformazioni isoterme, la temperatura non varia.
